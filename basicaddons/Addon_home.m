...
#define ADDON_BUNDLE [NSBundle bundleWithPath:@"/Library/Application Support/BeeKeyboard/Addons/Homescreen.bundle"]
#define LOCALIZED_TABLE_NAME @"Homescreen"
...
BOOL moveSBPage(id _event)
{
    ...
    NSString* pageString = [NSString stringWithFormat:@"Page %d", page];
    UIAccessibilityPostNotification(UIAccessibilityAnnouncementNotification, page == -1 ? LS(@"Spotlight") : [NSString stringWithFormat:@"%@ %@", LS(@"Move Page"), LS(pageString)]);
...
void movePageDirRight(BOOL right)
{
    ...
    if (index >= -1) {
        ...
        UIAccessibilityPostNotification(UIAccessibilityAnnouncementNotification, [NSString stringWithFormat:@"%@ %@", LS(@"Move Page"), LS(right ? @"To right" : @"To left")]);
    }
...
UIAccessibilityPostNotification(UIAccessibilityAnnouncementNotification, [[iconList objectAtIndex:jitterIndex] displayName]);
...
{
    SBAwayController* SBAC = (SBAwayController *)[objc_getClass("SBAwayController") sharedAwayController];
    if ([SBAC isLocked]) return 0;
    
    SpringBoard* sp = (SpringBoard *)[UIApplication sharedApplication];
    if ([sp _accessibilityFrontMostApplication] != nil) {
        return 0;
    }
    
    if (![NSStringFromClass([[[UIApplication sharedApplication] keyWindow] class]) isEqualToString:@"SBAppWindow"]) return 0;
